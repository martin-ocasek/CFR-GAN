...
from cfrgan.model.networks import CFRNet, TwoScaleDiscriminator
from cfrgan.model.losses import PerceptualLoss, WGAN_DIV_Loss
from cfrgan.tools.datasets import CFRDataset
from cfrgan.tools.logger import CFRLogger
import random
from cfrgan.backbone import IR_SE_50
...
class CFRTrain:
  ...
  def main(self):
      ...
      if self.args.multiprocessing_distributed:
          ...
          mp.spawn(self.main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, self.args))
      else:
          # Simply call main_worker function
          self.main_worker(self.args.gpu, ngpus_per_node, self.args)
  ...
  def main_worker(self, gpu, ngpus_per_node, args):
      ...
      if torch.distributed.get_rank() == 0:
          ...
          with mlflow.start_run(experiment_id=experiment.experiment_id, run_id="") as run:
              ...
              self.main_work(gpu, ngpus_per_node, args)
      else:
          self.main_work(gpu, ngpus_per_node, args)


  def main_work(self, gpu, ngpus_per_node, args):
      args.gpu = gpu
      ...
      for epoch in range(args.start_epoch, args.epochs):    
          ...
          if torch.distributed.get_rank() == 0:
              ...
              self.save_checkpoint(ckp, args.checkpoint_path, epoch, len(train_loader)*(epoch+1))
              self.mlflow_log_state_dict(ckp, artifact_path=f'checkpoint/ckp-{epoch}/')
              self.mlflow_log_state_dict(nets['G'].state_dict(), artifact_path=f'G/G-{epoch}/')
              torch.save(nets['G'].state_dict(), args.checkpoint_path + '/CFRNet_G_ep{}.pth'.format(epoch+1))

  def train(self, train_loader, valid_loader, nets, criterions, optimizers, schedulers, epoch, args):
      ...
      for i, (img, rot, gui, t_occ) in enumerate(train_loader):
          ...
          if i % args.n_critics == 0:
              ...
              if torch.distributed.get_rank() == 0:
                  ...
                  if i % args.log_iter == 0:
                      ...
                      self.logger.log_train_image(rotated_grid, guidance_grid, output_grid, occ_grid, target_grid, total_iter)

                  if i % 2 == 0:
                      sys.stdout.write("\r[Epoch %d/%d] [Batch %d/%d]" % (epoch, args.epochs, i, len(train_loader)))
                      self.logger.log_training(loss_D.item(), loss_GAN.item(), loss_occ_mask.item(), loss_per.item(), loss_rec.item(), loss_id.item(), total_iter)
          ...
          if torch.distributed.get_rank() == 0: 
              ...
              if i!=0 and i % args.check_iter == 0:
                  ...
                  self.save_checkpoint(ckp, args.checkpoint_path, epoch, len(train_loader)*(epoch+1))
                  self.mlflow_log_state_dict(ckp, artifact_path=f'checkpoint/ckp-{epoch}/',f=f"{i}.pth")
              ...
              if (i+1) % args.valid_iter == 0:
                  ...
                  with torch.no_grad():
                      ...
                      self.logger.log_test_image(img_grid, output_grid, rot_grid, gui_grid, mask_grid, total_iter)
                      
                      torch.cuda.empty_cache()                


  def save_checkpoint(self, state, save_path, epoch, iteration=None):
